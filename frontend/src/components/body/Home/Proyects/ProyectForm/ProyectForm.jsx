...
function ProyectForm() {
    ...
    const [selectedLangauges, setSelectedLanguages] = useState([]);
    ...
    useEffect(() => {
        const loadLanguages = async () => {
            ...
            setAllLanguages(result.response);
        };
        loadLanguages();
    }, []);
    ...
    useEffect(() => {
        if (isEdit) {
            const loadProyect = async () => {
                ...
                const { languages, ...rest } = result.response;
                setFormData(rest);
                const selectedLangObjects = allLanguages.filter(lang => languages.includes(lang._id));
    ...
    const handleSubmit = async (event) => {
        event.preventDefault();
        const languageIds = selectedLangauges.map(lang => lang._id);
        const dataToSend = {
            ...formData,
        ...
        let result;
        if (isEdit) {
            result = await fetchUpdateProyect(id, dataToSend);
        } else { result = await fetchCreateProyect(dataToSend); };
        ...
        <div id="formBodyGeneral">
            ...
            <form id="formGeneralContent" onSubmit={handleSubmit}>
                <div id="formGeneralContentBody">
                    ...
                    <LanguageSelect allLanguages={allLanguages} selectedLanguages={selectedLangauges} setSelectedLanguages={setSelectedLanguages} />
